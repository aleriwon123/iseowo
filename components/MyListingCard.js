...
export function MyListingCard ({listingDoc}) {
    ...
    const handleDeleteBusiness = async () => {
        ...
        await deleteDoc(doc(db, "directories", listingDoc.id))
        .then(() => {
            alert("Deleted successfully. Refresh page to propagate changes")
        })
        .catch(e => {
            alert("Request failed. Try again to contact the administrator")
            console.log(e)
        })
    }
    return (
        <div className=" relative flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 p-4 border border-lime-600 rounded-sm">
            ...
            <div className="flex items-center gap-3">
                ...
                }} className="md:min-w-[18px] flex justify-center items-center bg-lime-700 text-white px-2 rounded-sm">Update Listing</Link>
                
                <Link href="#" className="md:min-w-[18px] flex justify-center items-center bg-lime-700 text-white px-2 rounded-sm">View Listing</Link>
            </div>
            <CiTrash onClick={handleDeleteBusiness} className="absolute top-2 right-2 text-xl cursor-pointer"/>
        </div>
    )
}